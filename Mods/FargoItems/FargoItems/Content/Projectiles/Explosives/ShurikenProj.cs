        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    if ((x * x + y * y) <= radius)
                    {
                        if (tile.inActive() || FargoItems.Content.Logic.ExplosivesHelper.TileIsLiterallyAir(tile) || FargoItems.Content.Logic.ExplosivesHelper.TileBelongsToMagicStorage(tile))
                            continue;
