    ...
    internal class DevianttDialogueTracker
    {
        ...
        public void AddVanillaDialogue()
        {
            ...
            AddDialogue("WOF", HelpDialogueType.BossOrEvent,
                (name) => false && !Main.hardMode);

            AddDialogue("Deviantt", HelpDialogueType.BossOrEvent,
                (name) => NPC.downedBoss3 && !false);
            ...
            AddDialogue("Underwater", HelpDialogueType.Environment,
                (name) => !Main.LocalPlayer.GetJumpState(ExtraJump.Flipper).Enabled && !Main.LocalPlayer.gills);

            AddDialogue("Underworld", HelpDialogueType.Environment,
                (name) => !Main.LocalPlayer.fireWalk && !(Main.LocalPlayer.lavaMax > 0) && !Main.LocalPlayer.buffImmune[BuffID.OnFire]);

            AddDialogue("SpaceSuffocation", HelpDialogueType.Environment,
                (name) => !Main.LocalPlayer.buffImmune[BuffID.Suffocation]);

            //AddDialogue("The spirits of light and dark stopped by and they sounded pretty upset with you. Don't be too surprised if something happens to you for entering their territory!", HelpDialogueType.Environment, (name) => Main.hardMode);
