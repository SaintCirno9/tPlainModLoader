...
namespace Fargowiltas
{
    internal static class FargoUtils
    {
        ...
        public static bool EternityMode => false; // Fargowiltas.ModLoaded["FargowiltasSouls"] && (bool) 0;
        ...
        {
            if (npc != null)
                AddDebuffImmunities(npc.type, debuffs);
        }

        public static void AddDebuffImmunities(int npcType, List<int> debuffs)
        {
            if (debuffs == null || FargoSets.NPCs.SpecificDebuffImmunity == null) return;
            if (npcType >= 0 && npcType < FargoSets.NPCs.SpecificDebuffImmunity.Length)
            {
                if (FargoSets.NPCs.SpecificDebuffImmunity[npcType] == null)
                    FargoSets.NPCs.SpecificDebuffImmunity[npcType] = new Dictionary<int, bool>();

                foreach (int buffType in debuffs)
                {
                    FargoSets.NPCs.SpecificDebuffImmunity[npcType][buffType] = true;
                }
            }
        }
