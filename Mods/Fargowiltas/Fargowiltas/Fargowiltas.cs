...
namespace Fargowiltas
{
    public class Fargowiltas : Mod
    {
        ...
        public override void Load()
        {
            ...
            Terraria.On_Recipe.UpdateRecipeList += UpdateRecipeList_ElementalAssemblerGraveyardHack;
            Terraria.On_WorldGen.CountTileTypesInArea += CountTileTypesInArea_PurityTotemHack;
            Terraria.On_SceneMetrics.AggregateTileCounts += AggregateTileCounts_PurityTotemHack;
        ...
        private static void UpdateRecipeList_ElementalAssemblerGraveyardHack(
            Terraria.On_Recipe.orig_UpdateRecipeList orig)
        {
        ...
        //for current biome
        private static void AggregateTileCounts_PurityTotemHack(
            Terraria.On_SceneMetrics.orig_AggregateTileCounts orig,
            SceneMetrics self)
        {
            orig(self);
            ...
            if (self.GetTileCount((ushort)ModContent.TileType<PurityTotemSheet>()) > 0)
            {
                ...
                if (self.GetTileCount((ushort)TileID.Sunflower) > 0)
                    self.HasSunflower = true;
            }

            FargoWorld.CurrentSpawnRateTile ??= new bool[Main.netMode == NetmodeID.Server ? 255 : 1];
            ref bool current = ref FargoWorld.CurrentSpawnRateTile[0];
            bool oldSpawnRateTile = current;
            current = self.GetTileCount((ushort)ModContent.TileType<RegalStatueSheet>()) > 0;
        ...
        {
            // ModLoaded dictionary defaults to false for all unported external mods in TPML
            /*
            try
            {
                foreach (string mod in mods)
            ...
            */
            ...
            // Wikithis integration commented out for TPML
            /*
            if (false && !Main.dedServ)
            {
            ...
            */
            ...
            //    Main.RegisterItemAnimation(npc.Key, new DrawAnimationVertical(6, Main.npcFrameCount[npc]));
        ...
        internal static bool TryClearEvents()
        {
            ...
            if (canClearEvent)
            {
                ...
                if (Main.IsItRaining || Main.IsItStorming)
                {
                    ...
                    Main.cloudAlpha = 0;
                    if (Main.netMode == NetmodeID.Server)
                        if (Main.netMode == 2) NetMessage.SendData(53);
        ...
        //                loadInfo?.SetValue(null, true);

        //                /*foreach (Mod mod in ModLoader.Mods.Where(x => x != null))
        //                {
