...
namespace Fargowiltas.Items
{
    public class FargoGlobalItem : GlobalItem
    {
            ...
            if (FargoClientConfig.Instance.ExpandedTooltips)
            {
                ...
                if (FargoSets.Items.RegisteredShopTooltips[item.type] == null)
                {
                    ...
                    foreach (var shop in NPCShopDatabase.AllShops)
                    {
                        ...
                        {
                            Item npcItem = null;
                            foreach (var tryNPCItem in ContentSamples.ItemsByType.Where(i => i.Value.GetModItem() != null && i.Value.GetModItem() is CaughtNPCItem modItem && modItem.AssociatedNpcId == shop.NpcType))
                            {
                            ...
                            {
                                string grammar = i > 0 ? ", " : "";
                                conditions += grammar + condition.Description;
                                i++;
                            }
        ...
        public static void TryPiggyBankAcc(Item item, Player player)
        {
            ...
            if (FargoServerConfig.Instance.ModdedPiggyBankAcc && item.GetModItem() is ModItem modItem && modItem != null)
