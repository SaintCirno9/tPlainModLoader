...
namespace Fargowiltas.Items.Summons.Mutant
{
    public class AncientSeal : ModItem
    {
        public override void SetStaticDefaults()
        {
			...
			FargoSets.Items.SortingPriorityBossSpawns[Type] = 19; // Places it right after Celestial Sigil
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int i = NPCID.Count; i < NPCLoader.NPCCount; i++)
            {
                ...
                {
                    string name = npc.GetModNPC() != null ? npc.GetModNPC().DisplayName : Lang.GetNPCNameValue(npc.netID);
