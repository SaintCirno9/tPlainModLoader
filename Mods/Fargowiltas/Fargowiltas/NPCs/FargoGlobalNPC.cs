...
namespace Fargowiltas.NPCs
{
    public class FargoGlobalNPC : GlobalNPC
    {
        ...
        public override void ModifyShop(NPCShop shop)
        {
            ...
            if (FargoServerConfig.Instance.NPCSales)
            {
                ...
                void AddItem(int itemID, int customPrice = -1, Condition condition = null, Condition[] conditions = null)
                {
                    ...
                    {
                        if (customPrice != -1)
                            shop.Add(new Item() { type = itemID, shopCustomPrice = customPrice }, conditions);
                        else
                    ...
                    {
                        if (customPrice != -1)
                            shop.Add(new Item() { type = itemID, shopCustomPrice = customPrice });
                        else
                            shop.Add(itemID);
                    }
                }
            ...
            switch (npc.type)
            {
                case NPCID.Painter:
                    if (NPC.AnyNPCs(NPCID.MoonLordCore))
                        Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemType<EchPainting>());
                    break;
                ...
                case NPCID.DD2OgreT3:
                    if (!DD2Event.Ongoing)
                    {
                        if (Main.rand.NextBool(14))
                            Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.BossMaskOgre);

                        Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, Main.rand.Next(new int[] { ItemID.ApprenticeScarf, ItemID.SquireShield, ItemID.HuntressBuckler, ItemID.MonkBelt, ItemID.DD2SquireDemonSword, ItemID.MonkStaffT1, ItemID.MonkStaffT2, ItemID.BookStaff, ItemID.DD2PhoenixBow, ItemID.DD2PetGhost }));

                        Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.GoldCoin, Main.rand.Next(4, 7));
                    }
                    break;
                ...
                case NPCID.DD2DarkMageT3:
                    ...
                    {
                        if (Main.rand.NextBool(14))
                            Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.BossMaskDarkMage);

                        if (Main.rand.NextBool(10))
                            Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, Main.rand.NextBool() ? ItemID.WarTable : ItemID.WarTableBanner);

                        if (Main.rand.NextBool(6))
                            Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, Main.rand.Next(new int[] { ItemID.DD2PetGato, ItemID.DD2PetDragon }));
                    }
                    break;
                ...
                case NPCID.HeadlessHorseman:
                    ...
                    {
                        if (Main.rand.NextBool(10))
                            Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.JackOLanternMask);
                    }
                    break;
                ...
                case NPCID.MourningWood:
                    if (FargoUtils.ActuallyNight && !Main.pumpkinMoon)
                    {
                        Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.SpookyWood, 30);

                        Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, Main.rand.Next(new int[] {
                            ItemID.SpookyHook,
                            ItemID.SpookyTwig,
        ...
        public override bool CheckDead(NPC npc)
        {
            ...
            {
                Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.Wood, Main.rand.Next(10, 30));
            }
        ...
        private void Swarm(NPC npc, int boss, int minion, int bossbag, int trophy, int reward)
        {
            ...
            if (Fargowiltas.SwarmItemsUsed >= 10 && reward > 0)
                Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, reward, Fargowiltas.SwarmItemsUsed / 10);
                    

            //drop trophy for every 3 items
            if (Fargowiltas.SwarmItemsUsed >= 3 && trophy > 0)
                Item.NewItem(npc.GetSource_Loot(), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, trophy, Fargowiltas.SwarmItemsUsed / 3);
