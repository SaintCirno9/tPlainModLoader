    ...
    public class FargoGlobalProjectile : GlobalProjectile
    {
        ...
        public override Color? GetAlpha(Projectile projectile, Color lightColor)
        {
            if (lowRender && !projectile.hostile && FargoClientConfig.Instance.TransparentFriendlyProjectiles < 1)
            {
                ...
                Color? color = projectile.GetModProjectile()?.GetAlpha(lightColor);
                if (color != null)
                {
                    return color * FargoClientConfig.Instance.TransparentFriendlyProjectiles;
                }
        ...
        public static bool OkayToDestroyTile(Tile tile)
        {
            ...
            bool noDungeon = !NPC.downedBoss3 && (FargoSets.Walls.DungeonWall[tile.wall] || FargoSets.Tiles.DungeonTile[tile.type]);

            bool noHMOre = FargoSets.Tiles.HardmodeOre[tile.type] && !NPC.downedMechBossAny;
            ...
            if (noDungeon || noHMOre || noChloro || noLihzahrd || noAbyss || TileBelongsToMagicStorage(tile) ||
                FargoSets.Tiles.InstaCannotDestroy[tile.type] ||
                FargoSets.Walls.InstaCannotDestroy[tile.wall])
                return false;
        ...
        {
            return tile.type == 0 && tile.wall == 0 && tile.liquid == 0 /*&& tile.sTileHeader == 0 && tile.bTileHeader == 0 && tile.bTileHeader2 == 0 && tile.bTileHeader3 == 0*/ && tile.frameX == 0 && tile.frameY == 0;
        }

        public static bool TileBelongsToMagicStorage(Tile tile)
        {
            return false; // return Fargowiltas.ModLoaded["MagicStorage"] && ...
        }
    }
}
