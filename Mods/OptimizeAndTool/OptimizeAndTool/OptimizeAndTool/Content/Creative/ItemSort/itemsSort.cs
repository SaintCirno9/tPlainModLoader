...
namespace OptimizeAndTool.Content.Creative
{
    ...
    public partial class itemsSort
    {
        ...
        protected static void load_id_sort(Item i)
        {
            ...
            #endregion
            #region װ��
            else if (i.fishingPole > 0)//����
            {
                ItemsSort_Gets2(ID_ToolKit, ID_ToolKit_fishingPole)?.Items?.Add(i.type);
            }
            else if (load_id_sort_a1(i.shoot, Terraria.Main.projHook))//��צ
            {
                ItemsSort_Gets2(ID_ToolKit, ID_ToolKit_hook)?.Items?.Add(i.type);
            }
            else if (i.mountType != -1)//����
            {
                ItemsSort_Gets2(ID_ToolKit, ID_ToolKit_mount)?.Items?.Add(i.type);
            }
            else if (load_id_sort_a1(i.buffType, Terraria.Main.vanityPet) || load_id_sort_a1(i.buffType, Terraria.Main.lightPet))//����
            {
            ...
            {
                ItemsSort_Gets2(ID_BossSpawn)?.Items?.Add(i.type);//boss�ٻ���
            }
            else if (i.buffType != 0)//ҩˮ
            {
                ItemsSort_Gets2(ID_Buff)?.Items?.Add(i.type);
            }
            else if (i.consumable)//����Ʒ
            {
