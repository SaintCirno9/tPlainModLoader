...
namespace OptimizeAndTool.Content.QoL.Fishing
{
    ...
    /// </summary>
    [HarmonyPatch]
    internal class AutoFishingSupplies
    {
        ...
        [HarmonyPostfix]
        public static void PlayerUpdatePostfix(Player __instance)
        {
            if (__instance.whoAmI != Main.myPlayer || !__instance.active || __instance.dead)
                return;

            bool isFishing = HasActiveBobber(__instance);
            if (EnableAutoDrinkFishingBuffs.val && isFishing)
                MaintainBuffs(__instance);

            if (EnableAutoChumBuckets.val && isFishing)
                MaintainChum(__instance);
        }

        /// <summary>
        /// 跨容器寻找鱼饵：原版钓鱼条件只查背包，这里把虚空袋和随身银行也纳入
        /// </summary>
        [HarmonyPatch(typeof(Player), "Fishing_GetBait")]
        [HarmonyPostfix]
        public static void Fishing_GetBaitPostfix(Player __instance, ref int baitPower, ref int baitType)
        {
        ...
        [HarmonyPostfix]
        public static void UpdateEquipsPostfix(Player __instance)
        {
            if (!__instance.active || !EnableAnglerArmorVanityBonus.val)
                return;

            for (int i = 0; i < 3; i++)
            {
                int pieceId = i + ItemID.AnglerHat;
                if (__instance.armor[i].type == pieceId)
                    continue;

                Item candidate = null;
                if (__instance.armor[i + 10].type == pieceId)
                {
                    candidate = __instance.armor[i + 10];
                }
                else
                {
                    candidate = FindItemInInventoryOrVoidBag(__instance, pieceId);
                }

                if (candidate != null)
                {
                    __instance.GrantArmorBenefits(candidate);
                }
            }
        }
