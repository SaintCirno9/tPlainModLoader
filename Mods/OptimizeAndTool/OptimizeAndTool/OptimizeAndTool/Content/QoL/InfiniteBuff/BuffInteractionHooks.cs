...
{
    /// <summary>
    /// 原版 Buff 图标交互与提示渲染门控（基于 HookGen 强类型 On_ 门控）：
    ...
    /// 作者: SaintCirno9
    /// </summary>
    public static class BuffInteractionHooks
    {
        ...
        public static void RegisterAll()
        {
            ...
            On_Main.MouseText_DrawBuffTooltip += Hook_MouseText_DrawBuffTooltip;
            _registered = true;
        }
        ...
        {
            int result = orig(drawBuffText, buffSlotOnPlayer, x, y);

            if (Main.netMode == 2 || Main.gameMenu) return result;

            Player player = Main.LocalPlayer;
            if (player == null || buffSlotOnPlayer < 0 || buffSlotOnPlayer >= player.buffType.Length) return result;

            int buffType = player.buffType[buffSlotOnPlayer];
            if (buffType <= 0 || buffType >= TextureAssets.Buff.Length) return result;

            if (TextureAssets.Buff[buffType]?.Value == null) return result;
        ...
        private static void Hook_MouseText_DrawBuffTooltip(On_Main.orig_MouseText_DrawBuffTooltip orig, Main self, string buffString, ref int X, ref int Y, Vector2 mouseTextSize)
        {
