...
{
    public class skil12 : TPML.Content.ModPlayer
    {
        //技能12, 多个圈
        ...
        private static void a1_skil12_update_proj(Player player)
        {
            ...
            for (int i = 0; i < skil12_ps.Count; ++i)
            {
                if (Utils.projExist(skil12_ps[i], 459, player) == false)
                {
                    if (isOneSpawn)
                    {
                        int id = Projectile.NewProjectile(null, player.Center, Vector2.Zero, 459, SkilListControl1.damage.val, 1, player.whoAmI, ai1: skil12_size_this);
