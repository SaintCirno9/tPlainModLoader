...
namespace WandsTool.Content
{
    public partial class Wands
    {
        ...
        private static Point startTilePoint;
        private static int startTileType = -1;
        private static int startWallType = -1;
        public static int StartTileType => startTileType;
        public static int StartWallType => startWallType;
        ...
        private static void Update_Select()
        {
            ...
            if (Main.mouseLeft == true && Main.mouseLeftRelease == true && Main.mouseRight == false && selecting == false)
            {
                ...
                startTilePoint = position1.ToTileCoordinates();
                startTileType = -1;
                startWallType = -1;
                if (WorldGen.InWorld(startTilePoint.X, startTilePoint.Y))
                {
                    Tile st = Main.tile[startTilePoint.X, startTilePoint.Y];
                    if (st != null)
                    {
                        if (st.active()) startTileType = st.type;
                        if (st.wall > 0) startWallType = st.wall;
                    }
                }
            }
            ...
            if (Main.mouseLeft == false)
            {
                ...
                if (shapes != null && shapes.Count > 0)
                {
                    ...
                    {
                        WandAction.HandleBiomeConversion(shapes, GameMain.Wand_BiomeMode, GameMain.Wand_BiomeIncludeWall);
                    }
                    // 2. 液体魔杖操作
        ...
        public static void Draw()
        {
            ...
            else if (GameMain.Wand_BiomeMode != GameMain.BiomeMode.None)
            {
                switch (GameMain.Wand_BiomeMode)
                {
                    case GameMain.BiomeMode.Purity:
                        borderColor = new Color(100, 255, 120);
                        modeName = "纯净净化";
                        break;
                    case GameMain.BiomeMode.Corruption:
                        borderColor = new Color(170, 80, 255);
                        modeName = "腐化改造";
                        break;
                    case GameMain.BiomeMode.Hallow:
                        borderColor = new Color(100, 240, 255);
                        modeName = "神圣改造";
                        break;
                    ...
                    case GameMain.BiomeMode.Crimson:
                        borderColor = new Color(255, 60, 80);
                        modeName = "猩红改造";
                        break;
                    case GameMain.BiomeMode.Desert:
                        borderColor = new Color(255, 210, 80);
                        modeName = "沙漠改造";
                        break;
                    case GameMain.BiomeMode.Snow:
                        borderColor = new Color(180, 240, 255);
                        modeName = "冰雪改造";
                        break;
                    default:
        ...
        public static void DrawCursorModeTooltip(Microsoft.Xna.Framework.Graphics.SpriteBatch sb)
        {
            ...
            if (GameMain.Wand_BiomeMode != GameMain.BiomeMode.None)
            {
                string wallNotice = GameMain.Wand_BiomeIncludeWall ? " [含墙]" : " [仅物块]";
                switch (GameMain.Wand_BiomeMode)
                {
                    case GameMain.BiomeMode.Purity:
                        iconItemId = ItemID.GreenSolution; // 780
                        text = "纯净净化" + wallNotice;
                        textColor = new Color(100, 255, 120);
                        break;
                    case GameMain.BiomeMode.Hallow:
                        iconItemId = ItemID.BlueSolution; // 781
                        text = "神圣环境" + wallNotice;
                        textColor = new Color(100, 220, 255);
                        break;
                    case GameMain.BiomeMode.Corruption:
                        iconItemId = ItemID.PurpleSolution; // 782
                        text = "腐化环境" + wallNotice;
                        textColor = new Color(200, 100, 255);
                        break;
                    case GameMain.BiomeMode.Crimson:
                        iconItemId = ItemID.RedSolution; // 784
                        text = "猩红环境" + wallNotice;
                        textColor = new Color(255, 80, 100);
                        break;
                    case GameMain.BiomeMode.Mushroom:
                        iconItemId = ItemID.DarkBlueSolution; // 783
                        text = "发光蘑菇" + wallNotice;
                        textColor = new Color(80, 120, 255);
                        break;
                    case GameMain.BiomeMode.Desert:
                        iconItemId = ItemID.SandSolution; // 5392
                        text = "沙漠化" + wallNotice;
                        textColor = new Color(255, 220, 100);
                        break;
                    case GameMain.BiomeMode.Snow:
                        iconItemId = ItemID.SnowSolution; // 5393
                        text = "冰雪化" + wallNotice;
                        textColor = new Color(200, 240, 255);
                        break;
                    default:
                        iconItemId = ItemID.Clentaminator; // 779
                        text = "环境改造" + wallNotice;
                        textColor = Color.White;
                        break;
                }
            }
            // 2. 蓝图模式
            else if (GameMain.Wand_StructureMode != GameMain.StructureMode.None)
            {
