...
namespace WandsTool
{
    public class ListenInput : PatchMain
    {
        ...
        public override void UpdateUIStatesPostfix(Microsoft.Xna.Framework.GameTime gameTime)
        {
            Update_ListenInput();
        ...
        private static void Update_ListenInputOne()
        {
            ...
            {
                key = PlayerInput.GetPressedKeys()[0].ToString();
            }
            if (key == null && PlayerInput.MouseKeys.Count > 0)
            {
                key = PlayerInput.MouseKeys[0];
            }
