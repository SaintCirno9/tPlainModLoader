...
namespace Terraria.ModLoader.Assets
{
    ...
    public class ModAssetRepository
    {
        ...
        private Texture2D LoadTexture(string key)
        {
            ...
            if (bytes == null)
            {
                ModLoader.Log($"[ModAssetRepository] 未找到资产文件: {key}");
                return null;
            }
            ...
            if (device == null)
            {
                ModLoader.Log($"[ModAssetRepository] GraphicsDevice 尚未就绪: {key}");
                return null;
            }
            ...
            if (bytes.Length >= 12 && !IsPngHeader(bytes))
            {
                try
                {
                    ...
                    if (version == 1 && width > 0 && height > 0)
                    {
                        ...
                        if (bytes.Length >= 12 + expectedPixelBytes)
                        {
                            ...
                            ModLoader.Log($"[ModAssetRepository] 成功从 rawimg 载入纹理 [{key}] ({width}x{height})");
                            return tex;
                        }
                    }
                }
                ...
                {
                    ModLoader.Log($"[ModAssetRepository] 解析 rawimg [{key}] 异常: {ex.Message}");
                }
            }
            ...
            try
            {
                using (var ms = new MemoryStream(bytes))
                {
                    ...
                    ModLoader.Log($"[ModAssetRepository] 成功从 PNG 载入纹理 [{key}] ({tex.Width}x{tex.Height})");
                    return tex;
                }
            }
            catch (Exception ex)
            {
                ModLoader.Log($"[ModAssetRepository] 解析 PNG [{key}] 异常: {ex.Message}");
            }
