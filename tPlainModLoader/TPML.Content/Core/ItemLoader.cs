...
namespace Terraria.ModLoader
{
    ...
    public static class ItemLoader
    {
        ...
        public static int Register(ModItem item)
        {
            ...
            ModLoader.Log($"[ItemLoader] 成功注册物品: [{item.Mod?.Name ?? "Mod"}/{item.Name}] -> ItemID={type}");
            return type;
        }
        ...
        public static void LoadItemTexture(ModItem item)
        {
            try
            {
                ...
                ModLoader.Log($"[ItemLoader] ★ 物品 [{item.Name}] 贴图已绑定: {texture.Width}x{texture.Height}");
            }
            catch (Exception ex)
            {
                ModLoader.Log($"[ItemLoader] 载入物品贴图异常 [{item.Name}]: {ex.Message}");
            }
        }
        ...
        private static Asset<Texture2D> CreateAsset(Texture2D tex, string name)
        {
            ...
            try
            {
                ...
                ModLoader.Log($"[ItemLoader] ★ 成功为 [{name}] 构建有效 Asset<Texture2D> (尺寸: {tex.Width}x{tex.Height}, 状态: Loaded)");
                return asset;
            }
            catch (Exception ex)
            {
                ModLoader.Log($"[ItemLoader] CreateAsset 异常: {ex.Message}");
                return null;
            }
        }
