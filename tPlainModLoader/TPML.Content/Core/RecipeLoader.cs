    ...
    public static class RecipeLoader
    {
        ...
        public static void AddRecipes()
        {
            ...
            foreach (var item in ItemLoader.Items)
            {
                ...
                {
                    ModLoader.Log($"[RecipeLoader] 执行 {item.Name}.AddRecipes() 异常: {ex}");
                }
            }

            if (count > 0)
            {
                ModLoader.Log($"[RecipeLoader] 模组配方注册完成，本次处理 {count} 个物品。当前全局配方数: {Recipe.numRecipes}");
            }
        }
        ...
        public static void SetupRecipeLookups()
        {
            try
            {
                ...
                ModLoader.Log($"[RecipeLoader] ★ 已完成配方依赖关系与材料标记刷新 (numRecipes={Recipe.numRecipes})");
            }
            catch (Exception ex)
            {
                ModLoader.Log($"[RecipeLoader] SetupRecipeLookups 异常: {ex}");
            }
        }
        ...
        public static void RegisterRecipe(Recipe recipe)
        {
            ...
            try
            {
                ...
                {
                    ModLoader.Log($"[RecipeLoader] 跳过重复配方: 产物 ID={recipe.createItem.type}");
                    return;
                }
                ...
                ModLoader.Log($"[RecipeLoader] ★ 成功注入配方 #{assignedIndex}: 产物 [{itemName}] (ID={recipe.createItem.type}) x{recipe.createItem.stack}, 当前配方总数={Recipe.numRecipes}");
            }
            catch (Exception ex)
            {
                ModLoader.Log($"[RecipeLoader] RegisterRecipe 异常: {ex}");
            }
        }
