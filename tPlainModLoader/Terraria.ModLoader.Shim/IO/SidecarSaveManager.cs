    ...
    public static class SidecarSaveManager
    {
        ...
        public static void OnSavePlayerPrefix(PlayerFileData playerFile)
        {
            ...
            try
            {
                if (records.Count > 0)
                {
                    ...
                    TModShimEngine.Log($"[SidecarSave] 已导出 {records.Count} 个自定义物品至伴生存档: {Path.GetFileName(sidecarPath)}");
                }
            ...
            {
                TModShimEngine.Log($"[SidecarSave] 写入伴生存档异常: {ex.Message}");
            }
        }
        ...
        public static void OnLoadPlayerPostfix(Player player, string playerPath)
        {
            ...
            try
            {
                ...
                TModShimEngine.Log($"[SidecarSave] 成功从伴生存档还原 {restoredCount} 个自定义物品: {Path.GetFileName(sidecarPath)}");
            }
            catch (Exception ex)
            {
                TModShimEngine.Log($"[SidecarSave] 读取伴生存档异常: {ex.Message}");
            }
        }
    }
}
